...
/**
 * Says a claim is open, and does not say where.
 *
 * It carried a link to the claim message until it was pointed out that it had no business doing
 * so: `/voice claim` is the in-channel tier, so whoever ran it is already standing in the channel
 * the claim is in. A link to a message in the channel you are in says nothing. Pointing at the
 * button is both truer and cheaper.
 *
 ...
 */
const ClaimOpenEmbed = new EmbedBuilder( "VertixBot/UI-General/ClaimOpenEmbed" )
    .setInstanceType( UIInstancesTypes.Static )
    .setColor( VERTIX_DEFAULT_COLOR_BRAND )
    .setThumbnail( VERTIX_BRAND_THUMBNAIL_URL )
    .setTitle( "😈  This channel can be claimed" )
    .setDescription(
        "Its owner has left, so the channel is up for claiming.\n\n" +
        "Press **Claim** on this channel's buttons to start."
    )
    .build();

class ClaimOpenComponent extends UIComponentBase {
    public static getName() {
        return "VertixBot/UI-General/ClaimOpenComponent";
    }
    ...
    public static getEmbeds() {
        return [ ClaimOpenEmbed ];
    }
}

const ClaimOpenAdapter = new AdapterBuilderBase<
    TextChannel,
...
>( "VertixBot/UI-General/ClaimOpenAdapter", UIAdapterBase )
    .setComponent( ClaimOpenComponent )
