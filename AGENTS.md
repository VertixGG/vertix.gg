...
- **A guild settings row replaces the defaults rather than merging with them.** `GuildDataManager.getAllSettings()` returns the stored object whole when the row exists, so a row written with one field leaves every field added later reading as absent for that guild.
...
- Rewriting a language file programmatically: round-trip it first. `json.dumps( data, ensure_ascii = False, indent = 2 ) + "\n"` reproduces these files byte for byte — any other indent reformats all seven thousand lines and buries the actual change in the diff.
