...
> {
    ...
    public constructor( ... vars: TVars ) {
        ...
        vars.forEach( key =>
            varsWrappedObject[ key ] = UIEmbedVars.wrapAsVar( key )
        );
    ...
    public static wrapAsVar<const T extends string>( varName: T ) {
        return `${ UIEmbedVars.affix[ 0 ] }${ varName }${ UIEmbedVars.affix[ 1 ] }` as
            `${ typeof UIEmbedVars.affix[ 0 ] }${ T }${ typeof UIEmbedVars.affix[ 1 ] }`;
    }
