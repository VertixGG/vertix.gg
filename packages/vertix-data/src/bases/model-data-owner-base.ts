...
} from "@vertix.gg/data/src/factory/data-versioning-model-factory";

import type { TDataType, TDataDefaultResult } from "@vertix.gg/data/src/factory/data-type-factory";
...
> extends InitializeBase {
    ...
    protected async getWithOwner<T extends TDataType>(
        ...
        const keysWithOwner = await this.getUniqueKeys( keys, args, this.getWithOwner );
    ...
    protected async getAll<T extends TDataType>(
        ...
        if ( owners?.length ) {
            ...
            for ( const owner of owners ) {
                const keysWithOwner = { ...keys, ...this.getOwnerUniqueKeys( owner ) } as TWithOptionalProps<
                    TDataModelUniqueKeys,
                    "version"
                >;
