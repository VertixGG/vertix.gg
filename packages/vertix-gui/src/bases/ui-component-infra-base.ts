...
export abstract class UIComponentInfraBase extends UIPortableBase {
    ...
    protected static getInitialGroup(groupType: UIGroupsType, groups: (typeof UIGroupBase)[]) {
        ...
        throw new Error(
            `Initial group: '${initialGroup}' not found in: '${groupType}' groups, did you forget to add 'Group' suffix?`
        );
    }
    ...
    public switchElementsGroup(group: typeof UIElementsGroupBase | string) {
        ...
        staticThis.ensureEntities(
            staticThis.getFlatElements(this.currentElementsType.getItems() as UIElementsConstructor)
        );
    }
