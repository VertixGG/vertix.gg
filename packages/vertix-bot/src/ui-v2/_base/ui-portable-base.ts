...
import type { UIArgs, UIEntityTypes, UIPortableSchemaBase } from "@vertix.gg/bot/src/ui-v2/_base/ui-definitions";
...
export abstract class UIPortableBase<TSchema extends UIPortableSchemaBase = UIPortableSchemaBase> extends UIInstanceTypeBase {
    ...
    protected static ensureEntities( entities: UIEntityTypes = [], validateDuplicates = false ) {
        ...
        staticThis.portableDebugger.log( this.ensureEntities,
            `Validating entities for component: '${ this.getName() }'`
        );
        ...
        if ( this.isDynamic() ) {
            ...
            for ( const entity of entities ) {
                if ( entity.isStatic() ) {
                    throw new Error( `Entity: '${ entity.getName() }' is static, but component: '${ this.getName() }' is dynamic` );
                }
            }
        }
    ...
    protected async buildEntity( current: UIEntityBase | UIPortableBase, EntityClass: EntityPossibleTypes, onlyStatic = false, args?: UIArgs ) {
        ...
        if ( onlyStatic ) {
            ...
            if ( EntityClassType.isStatic() && this.isDynamic() ) {
                throw new Error( `Cannot create static entity: '${ EntityClassType.getName() }' in dynamic component: '${ this.getName() }'` );
            }
