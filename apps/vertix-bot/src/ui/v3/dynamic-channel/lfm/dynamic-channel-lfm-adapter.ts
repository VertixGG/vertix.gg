...
import { DynamicChannelLfmComponent } from "@vertix.gg/bot/src/ui/v3/dynamic-channel/lfm/dynamic-channel-lfm-component";
...
async function onNoteSubmitted(
    ...
    const noteInputId = context.customIdStrategy.generateId(
        "VertixBot/UI-V3/DynamicChannelLfmAdapter:VertixBot/UI-V3/DynamicChannelLfmNoteInput"
    );
    ...
    if ( DynamicChannelLfmPostResultCode.Cooldown === eligibility ) {
        ...
        await context.ephemeralWithStep( interaction, "VertixBot/UI-V3/DynamicChannelLfmCooldown", {
    ...
    if ( DynamicChannelLfmPostResultCode.Success !== eligibility ) {
        await context.ephemeralWithStep( interaction, "VertixBot/UI-V3/DynamicChannelLfmUnavailable", {
    ...
    await context.ephemeralWithStep( interaction, "VertixBot/UI-V3/DynamicChannelLfmSelectChannel", {
...
async function postAndAnswer<TInteraction extends DefaultInteraction>(
    ...
    await context.ephemeralWithStep( interaction, "VertixBot/UI-V3/DynamicChannelLfmPosted", {
        lfmChannelId
    } );
}

const DynamicChannelLfmAdapter = new DynamicExecutionAdapterBuilder<DefaultInteraction>(
    "VertixBot/UI-V3/DynamicChannelLfmAdapter"
)
    ...
    .defineTransactions( ( tx ) => {
        tx
            ...
            .addState( "Default", {
                ...
                elementsGroup: "VertixBot/UI-V3/DynamicChannelLfmChannelMenuGroup"
            } )
            .addState( "SelectChannel", {
                executionStep: "VertixBot/UI-V3/DynamicChannelLfmSelectChannel",
                navigationType: "ephemeral",
                embedsGroup: "VertixBot/UI-V3/DynamicChannelLfmSelectChannelEmbedGroup",
                elementsGroup: "VertixBot/UI-V3/DynamicChannelLfmChannelMenuGroup"
            } )
            .addState( "Posted", {
                executionStep: "VertixBot/UI-V3/DynamicChannelLfmPosted",
                navigationType: "ephemeral",
                previewDefaultVars: { lfmChannelId: "123456789" },
                embedsGroup: "VertixBot/UI-V3/DynamicChannelLfmPostedEmbedGroup"
            } )
            .addState( "Cooldown", {
                executionStep: "VertixBot/UI-V3/DynamicChannelLfmCooldown",
                navigationType: "ephemeral",
                previewDefaultVars: { retryMinutes: "10" },
                embedsGroup: "VertixBot/UI-V3/DynamicChannelLfmCooldownEmbedGroup"
            } )
            .addState( "Unavailable", {
                executionStep: "VertixBot/UI-V3/DynamicChannelLfmUnavailable",
                navigationType: "ephemeral",
                previewDefaultVars: { reasonCode: String( DynamicChannelLfmPostResultCode.NotConfigured ) },
                embedsGroup: "VertixBot/UI-V3/DynamicChannelLfmUnavailableEmbedGroup"
            } )
            ...
            .addTransition( "SelectDestination", {
                from: "Default",
                to: "SelectChannel",
                // Which way a note goes is decided by how many boards an admin set up, and that is
                // a generator's settings rather than anything typed - so there is nothing in a
                // submitted note for a preview to read. It counts the boards instead, the way the
                // rename screen counts attempts to reach a limit it cannot otherwise show.
                previewCondition: { field: "destinations", operator: "equals", value: "many" }
            } )
            .addTransition( "ChannelSelected", {
                from: "SelectChannel",
                to: "Posted",
                // The board that was picked, which is what the closing screen names. Declared the
                // same way the one-destination path declares it: both arrive at the same state and
                // it has to be able to say where the post went either way round.
                mutations: [ { type: "set", path: [ "lfmChannelId" ] } ]
            } )
            ...
            .bindModal<UIDefaultModalChannelVoiceInteraction>(
                "VertixBot/UI-V3/DynamicChannelLfmNoteModal",
                "NoteSubmitted",
                onNoteSubmitted
            );
    } )
    .getReplyArgs( async( context, interaction, argsFromManager ) => {
        switch ( context.getCurrentExecutionStep( interaction )?.name ) {
            case "VertixBot/UI-V3/DynamicChannelLfmSelectChannel":
                return { lfmDestinations: argsFromManager?.lfmDestinations };

            case "VertixBot/UI-V3/DynamicChannelLfmPosted":
                return { lfmChannelId: argsFromManager?.lfmChannelId };

            case "VertixBot/UI-V3/DynamicChannelLfmCooldown":
                return { retryMinutes: argsFromManager?.retryMinutes };

            case "VertixBot/UI-V3/DynamicChannelLfmUnavailable":
