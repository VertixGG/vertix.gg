...
export class UIHashService extends ServiceBase {
    ...
    public generateId(
        ...
        if (parted.length > 1) {
            this.debugger.log(this.generateId, "Generating custom id for parted id:", {
                id,
                parted,
                separator,
                maxLength
        ...
        this.debugger.log(
            this.generateId,
            `Generated hash id: '${hash.slice(0, 32) + "..." + hash.slice(hash.length - 4)}' for id: '${id}'`
        );
    ...
    public getId(
        ...
        if (hashedParts.length > 1) {
            this.debugger.log(this.getId, "Getting id from hashed parts:", hashedParts);

            const result = hashedParts
                .map((part) =>
                    this.getId(part, separator, {
            ...
            this.debugger.log(this.getId, "Got id:", { hashedParts, result, resultLen: result.length });
    ...
    public loadTablesFromFile(filePath = process.cwd() + "/ui-hash-tables.json") {
        ...
        this.hashTable = new Map(
            data.hashTable.map(([length, entries]: [number, [string, string][]]) => [length, new Map(entries)])
        );
        this.hashTableReverse = new Map(
            data.hashTableReverse.map(([length, entries]: [number, [string, string][]]) => [length, new Map(entries)])
        );
    ...
    public async saveTablesToFile(filePath = process.cwd() + "/ui-hash-tables.json") {
        ...
        const data = {
            ...
            hashTableReverse: Array.from(this.hashTableReverse.entries()).map(([length, map]) => [
                length,
