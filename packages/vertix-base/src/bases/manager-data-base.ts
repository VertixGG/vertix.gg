...
> extends CacheBase<DataResult> implements IDataManager {
     ...
     *
     *         this.logger.log(this.getData, this.getLogMessage("Getting", ownerId, key, cache));
     *
     *         const cachedData = this.getCacheData(cache, cacheKey, ownerId, key);
     *         if (cachedData) {
     *             return cachedData;
     *         }
     *
     *         const dbData = await this.getDatabaseData(args);
     *         if(!dbData) {
     *             return;
     *         }
     *
     *         const [data, message] = await this.createOrGetData(dbData, args);
     *         if(!data) {
     *             return;
     *         }
     *
     *         this.debugger.log(this.getData, `${message} data for ownerId: '${args.ownerId}', type: '${data.type}' key: '${args.key}' values:\n`, data.object || data.values );
     *
     ...
     *     }
     *
     *     private getLogMessage(type: string, ownerId: string, key: string, cache: boolean) {
     *         return `${type} data for ownerId: '${ownerId}' key: '${key}' cache: '${cache}'`;
     *     }
     *
     *     private getCacheData(cache: boolean, cacheKey: string, ownerId: string, key: string) {
     *         if (cache) {
     *             const cached = this.getCache(cacheKey);
     *             if (cached) {
     *                 this.debugger.log(this.getData, this.getLogMessage("Getting cached", ownerId, key, "- Returned from: cache"));
     *                 return cached;
     *             }
     *         }
     *         return null;
     *     }
     *
     *     private async getDatabaseData(args: IDataGetArgs) {
     *         const dbData = await this.dataSourceModel.getData(args).catch(() => {});
     *         if (!dbData) {
     *             this.logger.debug(this.getData, this.getLogMessage("Could not find", args.ownerId, args.key, ""));
     *         }
     *         return dbData;
     *     }
     *
     *     private async createOrGetData(dbData: any, args: IDataGetArgs): Promise<[DataResult, string]> {
     ...
     *             message = "Getting";
     *         } else {
     *             this.logger.debug(this.getData, this.getLogMessage("Could not find", args.ownerId, "", ""));
     *             return null;
     *         }
     *
     *         return [data, message];
