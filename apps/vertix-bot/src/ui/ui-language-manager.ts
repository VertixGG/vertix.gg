...
export class UILanguageManager extends InitializeBase implements UILanguageManagerInterface {
    ...
    // In-memory lookup maps: lang code -> entity name -> content
    private buttonsByLang = new Map<string, Map<string, UIElementButtonLanguageContent>>();
    private selectMenusByLang = new Map<string, Map<string, UIElementSelectMenuLanguageContent>>();
    private textInputsByLang = new Map<string, Map<string, UIElementTextInputLanguageContent>>();
    private embedsByLang = new Map<string, Map<string, UIEmbedLanguageContent>>();
    private markdownsByLang = new Map<string, Map<string, UIMarkdownLanguageContent>>();
    ...
    public async getSelectMenuTranslatedContent(
        ...
        return {
            ...
            selectOptions: content.selectOptions as any,
            options: content.options as any
        };
    }
    ...
    public async getEmbedTranslatedContent(
        ...
        return {
            ...
            options: content.options as any,
            arrayOptions: content.arrayOptions as any
        };
    }
    ...
    public async register( options: {
        ...
        // TODO: Use checksum to avoid extra validations.
        if ( fs.existsSync( UI_LANGUAGES_INITIAL_FILE_PATH ) ) {
            this.logger.info(
                this.register,
                `Initial language code '${ UI_LANGUAGES_INITIAL_CODE }' exists, validating...`
            );
            ...
            // Build in-memory lookup maps from loaded JSON files.
    ...
    private validateLanguage(
        ...
        const validateElementCount = ( elementType: string, currentArray: any[], sourceArray: any[] ) => {
            ...
            if ( currentCount !== sourceCount ) {
                const message =
                    `Language code: '${ currentLanguage.code }' has a different ${ elementType } ` +
    ...
    private validateElements<T extends { name: string; content: any }>(
        ...
        for ( const initialElement of initialElements ) {
            ...
            if ( !currentElement ) {
                throw new Error(
                    `Language code: '${ currentLanguage.code }' does not have ${ elementName } with name: '${ initialElement.name }', if it's changed you have to update language in files and DB`
                );
            }

            if ( elementName === "button" || elementName === "textInput" ) {
                const validLabel = this.validateString(
                    initialElement.content.label,
                    currentElement.content.label,
                    options
                );
                if ( validLabel !== "valid" ) {
                    throw new Error(
                        `Language code: '${ currentLanguage.code }' ${ elementName } with name: '${ initialElement.name }' has invalid attribute: 'label' code: '${ validLabel }'.`
                    );
                }
            }

            if ( elementName === "textInput" || elementName === "selectMenu" || elementName === "embed" ) {
                const validPlaceholder = this.validateString(
                    initialElement.content.placeholder,
            ...
            if ( elementName === "embed" || elementName === "markdown" ) {
                const validContent = this.validateString(
                    initialElement.content.content,
            ...
            if ( initialOptionsLength !== currentOptionsLength ) {
                throw new Error(
                    `Language code: '${ currentLanguage.code }' ${ elementName } with name: '${ initialElement.name }' has different options count - '${ initialOptionsLength }' !== '${ currentOptionsLength }'.`
                );
            }
            ...
            if ( elementName === "embed" && typeof initialElement.content.arrayOptions === "object" ) {
                ...
                if ( initialKeys !== keys ) {
                    throw new Error(
                        `Language code: '${ currentLanguage.code }' embed with name: '${ initialElement.name }' has different array options keys`
                    );
                }
            }
            ...
            if ( !options.skipSameValues && initialElement.content.options && currentElement.content.options ) {
                ...
                if ( "{}" !== initialOptions && "{}" !== currentOptions && initialOptions === currentOptions ) {
                    throw new Error(
                        `Language code: '${ currentLanguage.code }' ${ elementName } with name: '${ initialElement.name }' has the same options as initial language.`
                    );
                }
            }
        }
    ...
    private validateElementsLanguage(
        ...
        this.validateElements(
            currentLanguage,
            currentLanguage.elements.buttons,
            initialLanguage.elements.buttons,
            "button",
            options
        );
        this.validateElements(
            currentLanguage,
            currentLanguage.elements.textInputs,
            initialLanguage.elements.textInputs,
            "textInput",
            options
        );
        this.validateElements(
            currentLanguage,
            currentLanguage.elements.selectMenus,
            initialLanguage.elements.selectMenus,
            "selectMenu",
            options
        );
    }
    ...
    private validateMarkdownsLanguage(
        ...
        this.validateElements(
            currentLanguage,
            currentLanguage.markdowns,
            initialLanguage.markdowns,
            "markdown",
            options
        );
    }
    ...
    /**
     * Builds in-memory lookup maps from all loaded language JSON files.
