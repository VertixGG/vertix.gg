...
/**
 * Told to whoever ran an admin command without the permissions for it.
 *
 * Its own words rather than `MissingPermissionsAdapter`, which means the *bot* is missing
 * permissions - the two read almost alike and mean opposite things.
 *
 ...
 */
const MissingAdminPermissionsEmbed = new EmbedBuilder( "VertixBot/UI-General/MissingAdminPermissionsEmbed" )
    .setInstanceType( UIInstancesTypes.Static )
    .setColor( VERTIX_DEFAULT_COLOR_BRAND )
    .setThumbnail( VERTIX_BRAND_THUMBNAIL_URL )
    .setTitle( "⛔  You cannot configure this server" )
    .setDescription(
        "This command is for server admins.\n\n" +
        "It needs the **Manage Server**, **Manage Channels** and **Manage Roles** permissions."
    )
    .build();

class MissingAdminPermissionsComponent extends UIComponentBase {
    public static getName() {
        return "VertixBot/UI-General/MissingAdminPermissionsComponent";
    }
    ...
    public static getEmbeds() {
        return [ MissingAdminPermissionsEmbed ];
    }
}

const MissingAdminPermissionsAdapter = new AdapterBuilderBase<
    TextChannel,
...
export { MissingAdminPermissionsAdapter };
