 ...
 *
 * That defence works for a press and not for a command, which carries no message and falls through
 * to the rewritten `channelId`. Nothing decides behaviour on this any more - what an interaction
 * means is declared rather than read off it, in `interaction-channel-context.ts`. This is now only
 * a lookup: where did this happen.
 */
